...
def go_to_mine(
    ...
    *,
    ...
    steps = [
        WorkflowStep.tap(POS_MINE[0], POS_MINE[1], wait_after=2),
        WorkflowStep.tap(POS_MINE_FIRST[0], POS_MINE_FIRST[1], wait_after=2),
    ]
    return not runner.run(steps, stop_event=stop_event, log_fn=log_fn)


def start_beiguo_comment(
    adb_serial: str,
    *,
    stop_event: threading.Event | None = None,
    log_fn: Callable[[str], None] | None = None,
    ...
    while not _should_stop(stop_event):
        ...
        if where == "send":
            ...
            go_to_mine(runner, stop_event=stop_event, log_fn=log_fn, adb_serial=adb_serial)
